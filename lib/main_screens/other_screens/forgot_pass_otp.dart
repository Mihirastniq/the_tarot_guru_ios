...
import 'package:the_tarot_guru/main_screens/other_screens/new_pass.dart';
...
class _ForgotPasswordOTPVerifyState extends State<ForgotPasswordOTPVerify> {
  ...
  void _onContinuePress() async {
    ...
    int otpFromResponse = widget.otp; // Get the OTP from the response object
    ...
    if (pin == otpFromResponseString) { // Compare the entered OTP with the OTP from the response
      Navigator.push(
        context,
        MaterialPageRoute(builder: (context) => SetNewPasswordScreen(email: widget.email)),
      );
    } else {
